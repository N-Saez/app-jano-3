...
APP_VERSION = "v1.18f FIN" 
st.set_page_config(page_title=f"Dilución Geológica {APP_VERSION}", layout="wide")
# tratando de arreglar el máximo de tramo diliuble (pedida)
# --------------------- Backend Functions --------------------------
...
def generate_block_based_code_reassignment_trace(
    ...
    geocode_col,
    absorb_lengths_dict,
    affinity_matrix_bool,
    activar_absorcion_bilateral,
    multiplicador_X,
    manejo_ignorados,
    codigos_marcados_como_ignorados,
    max_gap_dist,
    default_absorb_length_for_ignored_B,
    max_absorb_length=float('1000.0'),
    max_iter=1
):
    # --- Caso vacío y validaciones iniciales (idéntico al original) ---
    ...
    if not all(c in df_trace_processing.columns for c in [hole_col, from_col, to_col, geocode_col]):
        st.error("GBB: Faltan columnas esenciales.")
        return df_input_trace
    try:
        ...
        st.error(f"GBB: Error FROM/TO a numérico: {e}")
        return df_input_trace

    df_trace_processing['original_interval_id'] = df_trace_processing.index
    df_trace_processing['interval_length'] = (
        df_trace_processing[to_col] - df_trace_processing[from_col]
    )
    df_valid_intervals = df_trace_processing[
        df_trace_processing['interval_length'] > 0
    ].copy()
    ...
    # Inicializar debug_data para todos los intervalos
    debug_data = {
        oid: {
            'debug_block_id':      -99,
            'debug_block_code':    '-99',
            'debug_prev_block_code': '-99',
            'debug_next_block_code': '-99',
            'debug_block_len':     -99,
            'debug_prev_block_id': -99,
            'debug_prev_block_len':-99,
            'debug_next_block_id': -99,
            'debug_next_block_len':-99,
            'debug_target_code':   '-99',
            'debug_absorbed':      False,
            'debug_reason':        'Sin condiciones de bilateralidad-largo'
        }
        for oid in iter_codes.index
    }
    ...
    for iter_num in range(1, max_iter + 1):
        ...
        for hole_id in all_hole_ids:
            hole_df = df_valid_intervals[df_valid_intervals[hole_col] == hole_id] \
                      .sort_values(by=from_col)
            if hole_df.empty:
                continue

            idx_list = hole_df['original_interval_id'].tolist()
            if not idx_list:
                continue

            codes_start = iter_codes.loc[idx_list]
            lengths_start = hole_df.set_index('original_interval_id')['interval_length'] \
                                   .reindex(idx_list)
            from_start = hole_df.set_index('original_interval_id')[from_col] \
                                .reindex(idx_list)
            to_start = hole_df.set_index('original_interval_id')[to_col] \
                              .reindex(idx_list)

            # --- Construcción inicial de bloques ---
            blocks = []
            curr_inds, curr_code, curr_len = [], None, 0.0
            last_to = -float('inf')

            for oid in idx_list:
                code = codes_start.loc[oid]
                length = lengths_start.loc[oid]
                gap = (from_start.loc[oid] - last_to) if curr_inds else 0

                if not curr_inds or code != curr_code or gap > max_gap_dist:
                    if curr_inds:
                        blocks.append({
                            'indices': curr_inds.copy(),
                            'code':    curr_code,
                            'length':  curr_len,
                            'from':    from_start.loc[curr_inds[0]],
                            'to':      to_start.loc[curr_inds[-1]]
                        })
                    curr_inds = [oid]
                    curr_code = code
                    curr_len  = length
                else:
                    curr_inds.append(oid)
                    curr_len += length

                last_to = to_start.loc[oid]
            ...
            if not blocks:
                continue
            # Clonar códigos actuales a snapshot
            snapshot = new_codes_for_iter_main.loc[idx_list].copy()      
            # --- FUSIÓN BILATERAL DINÁMICA con debug---
            if activar_absorcion_bilateral:
                ...
                while i < len(blocks):
                    if iter_num == 1:
                        ...
                        curr_blk = blocks[i]
                        # Registro de debug para bloques iniciales
                        for oid in curr_blk['indices']:
                                    debug_data[oid].update({
                                    'debug_block_id':       i,
                                    'debug_block_code':     curr_blk['code'],
                                    'debug_prev_block_code': 'SVP',
                                    'debug_next_block_code': 'SVS',
                                    'debug_block_len':      curr_blk['length'],
                                    'debug_prev_block_id':  i-1,
                                    'debug_prev_block_len': -99,
                                    'debug_next_block_id':  i+1,
                                    'debug_next_block_len': -99,
                                    'debug_target_code':    '-99',
                                    'debug_absorbed':       False,
                                    'debug_reason':         'no cumple condición de vecinos'
                                    })
                    ...
                    if i > 0 and i + 1 < len(blocks):
                        ...
                        curr_blk = blocks[i]
                        next_blk = blocks[i + 1]
                        is_ignored = str(curr_blk['code']) in codigos_marcados_como_ignorados
                        min_len = absorb_lengths_dict.get(curr_blk['code'], float('inf'))
                        if is_ignored and manejo_ignorados == 'B' and curr_blk['code'] not in absorb_lengths_dict:
                            min_len = default_absorb_length_for_ignored_B

                        is_ignorado_fijo = (manejo_ignorados == 'A' and str(curr_blk['code']) in codigos_marcados_como_ignorados)
                        is_ignorado_absorbible = (manejo_ignorados == 'B' and str(curr_blk['code']) in codigos_marcados_como_ignorados)
                        is_prev_noabsorvente = (manejo_ignorados in ("A","B") and str(prev_blk['code']) in codigos_marcados_como_ignorados)
                         
                        if ((not is_ignorado_fijo) and (not is_prev_noabsorvente)
                            and (curr_blk['length'] <= max_absorb_length)
                            and  (curr_blk['length'] > min_len)
                            and prev_blk['code'] == next_blk['code']
                            and (prev_blk['length'] >= multiplicador_X * curr_blk['length'])
                            and (next_blk['length'] >= multiplicador_X * curr_blk['length'])
                            and affinity_matrix_bool.loc[prev_blk['code'], curr_blk['code']]
                            and affinity_matrix_bool.loc[next_blk['code'], curr_blk['code']]
                        ):
                            # Registro de debug para intervalos absorbidos
                            if iter_num == 1:
                                cur_len  = curr_blk['length']
                                prev_len = prev_blk['length']
                                next_len = next_blk['length']
                                
                                for oid in curr_blk['indices'] + next_blk['indices']:
                                    debug_data[oid].update({
                                        'debug_block_id':      i,
                                        'debug_block_code':    curr_blk['code'],
                                        'debug_prev_block_code': prev_blk['code'],
                                        'debug_next_block_code': next_blk['code'],
                                        'debug_block_len':     cur_len,
                                        'debug_prev_block_id': i-1,
                                        'debug_prev_block_len':prev_len,
                                        'debug_next_block_id': i+1,
                                        'debug_next_block_len':next_len,
                                        'debug_target_code':   prev_blk['code'],
                                        'debug_absorbed':      True,
                                        'debug_reason':        'absorbido bilateral'
                                    })
                             # Reasignar snapshot y marcar cambio       
                            for oid in curr_blk['indices'] + next_blk['indices']:
                                snapshot.loc[oid] = prev_blk['code']
                                changed_in_this_main_iter = True
                            # Fusionar curr + next en prev
                            prev_blk['indices'] += curr_blk['indices'] + next_blk['indices']
                            prev_blk['length']  += curr_blk['length'] + next_blk['length']
                            prev_blk['to']       = next_blk['to']

                            # Eliminar bloques absorbidos
                            blocks.pop(i + 1)  # next
                            blocks.pop(i)      # curr

                            # Retroceder para re-evaluar
                            i = max(i - 1, 0)                            
                        else:
                             # ---- DEBUG NO absorción ----
                            if iter_num == 1:
                                prev_len, next_len, block_len = prev_blk['length'], next_blk['length'], curr_blk['length']
                                for oid in curr_blk['indices']:
                                    debug_data[oid].update({
                                    'debug_block_id':       i,
                                    'debug_block_code':     curr_blk['code'],
                                    ...
                                    'debug_target_code':    '-99',
                                    'debug_absorbed':       False,
                                    'debug_reason':         'no cumple condición bilateral'
                                    })
                                i += 1 
                    else:
                        i += 1

            # --- Reasignación final de bloques no fusionados ---
            for blk_id, blk in enumerate(blocks):
                code_blk = blk['code']
                for oid in blk['indices']:
                    if snapshot.loc[oid] != code_blk:
                        snapshot.loc[oid] = code_blk
                        changed_in_this_main_iter = True
            

            new_codes_for_iter_main.loc[idx_list] = snapshot

        df_iter_results[iter_name] = new_codes_for_iter_main
        iter_codes = new_codes_for_iter_main.copy()
        if not changed_in_this_main_iter:
            st.success(f"Convergencia en iteración {iter_num}.")
            break

        if iter_num == max_iter:
            st.warning(f"Máx. iteraciones ({max_iter}) alcanzado.")

    df_iter_results["geocode_iter_final"] = iter_codes

    # --- Incorporar debug_data al DataFrame de resultados ---
    # vaciando para no dibujar en la salida
    debug_data={}
    if debug_data:
        df_dbg = pd.DataFrame.from_dict(debug_data, orient="index")
        cols_dbg = [
        "debug_prev_block_id","debug_block_id","debug_next_block_id",  
        "debug_prev_block_code","debug_block_code","debug_next_block_code",
        "debug_prev_block_len","debug_block_len","debug_next_block_len",
        "debug_target_code","debug_absorbed","debug_reason"
        ]
        for col in cols_dbg:
            if col not in df_dbg.columns:
                df_dbg[col] = np.nan
        for col in cols_dbg:
            df_iter_results[col] = df_dbg[col]

    # --- Armado del DataFrame de salida y retorno ---
    df_output_trace = df_input_trace.copy()
    for col in df_iter_results.columns:
        if col == geocode_col and geocode_col in df_output_trace.columns:
            df_output_trace[col].update(df_iter_results[col])
        else:
            df_output_trace[col] = df_iter_results[col].reindex(df_output_trace.index)
        if geocode_col in df_output_trace.columns:
            mask = df_output_trace[col].isna()
            df_output_trace.loc[mask, col] = df_output_trace.loc[mask, geocode_col]
        else:
            df_output_trace[col].fillna("INDEFINIDO", inplace=True)
...
if "fase" not in st.session_state:
    ...
    st.session_state.max_iterations_reassign = 1
    st.session_state.activar_absorcion_bilateral = True 
...
with cont0:
    ...
    if uploaded_file and (st.session_state.df_original is None or uploaded_file.name != st.session_state.uploaded_file_name):
        try:
            ...
            st.session_state.activar_absorcion_bilateral = True; st.session_state.multiplicador_X_absorcion = 1.0
...
if st.session_state.fase >= 2 and not st.session_state.equiv_final.empty:
    with cont2:
        st.header("Fase 2: Configuración de Longitud Mínima de Absorción (`min_absorb_length`)")
        ...
        if not codes_for_absorb_df: st.warning("No hay códigos activos (no ignorados) para configurar `absorb_length`. Revise Fase 1.")
            ...
            st.session_state.max_absorb_length_global = st.number_input("Máx. Largo Aceptable por Tramo (Global):",min_value=0.0, value=1000.0, step=1.0, format="%.2f",key=f"num_maxlen_{APP_VERSION}",help="Tramos más largos que este valor (en metros) no se diluyen ni se consideran en la reasignación. Este valor se aplica globalmente.")  # [CAMBIO: max_absorb_length global - movido a Fase 2]
...
if st.session_state.fase >= 3: 
    with cont3:
        ...
        if st.session_state.absorb_df.empty: st.warning("Complete Fase 2 (Absorb_length) primero.")
            ...
            st.session_state.max_iterations_reassign = st.number_input("Máx. iteraciones para Reasignación:",min_value=1,max_value=1,value=st.session_state.get("max_iterations_reassign",1),step=1,key=f"max_iter_reassign_f3_{APP_VERSION}")
            st.markdown("---") 
...
if st.session_state.fase >= 4:
    if st.session_state.get('df_block_reassigned_trace') is None and st.session_state.fase == 4:
        with cont4:
            st.header("Fase 4: Ejecutando Dilución-Bilateral-reasignación de Códigos")
            ...
            max_iter_f4 = st.session_state.get("max_iterations_reassign", 1)
            act_bilateral_f4 = True #st.session_state.get("activar_absorcion_bilateral", False)
            ...
            max_absorb_length_f4 = st.session_state.get("max_absorb_length_global", float('1000.0'))  # [CAMBIO: max_absorb_length global]
            ...
            try:
                ...
                df_trace_res = generate_block_based_code_reassignment_trace(
                    ...
                    manejo_ign_f4, codes_ign_list_f4, max_gap_f4, default_abs_len_ign_f4, max_absorb_length_f4, max_iter_f4)
...
if st.session_state.fase >= 5:
    with cont5:
        ...
        if df_trace_display_f5 is not None and not df_trace_display_f5.empty:
            ...
            debug_cols_f5 = [c for c in df_trace_display_f5.columns if c.startswith("debug_")]
            display_cols_f5.extend(debug_cols_f5)
