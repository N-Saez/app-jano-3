...
# Los 5 escenarios a dibujar (coordenadas REALES en metros, una franja
# inclinada ~37° distinta por escenario, como las usadas en las pruebas)
def escenarios_demo():
    out = []
    for k in range(5):
        w = 110 + 25 * k
        out.append([(150, 80 + 25 * k), (900, 680 + 25 * k),
                    (900 - w, 680 + w + 25 * k), (150, 80 + w + 25 * k)])
    return out
...
def main():
    with sync_playwright() as p:
        ...
        for i, coords in enumerate(escenarios_demo(), start=1):
            print(f"  Escenario {i}")
